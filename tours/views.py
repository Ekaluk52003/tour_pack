...
def get_city_services(request, city_id):
    """
    API endpoint to get services for a specific city.
    """
    try:
        tour_pack_type_id = request.GET.get('tour_pack_type')
        city = City.objects.get(id=city_id)
        tour_pack_type = TourPackType.objects.get(id=tour_pack_type_id)
        ...
        return JsonResponse(response_data)
    except (City.DoesNotExist, TourPackType.DoesNotExist):
        return JsonResponse({'error': 'City or Tour Package Type not found'}, status=404)
    except Exception as e:
        return JsonResponse({'error': str(e)}, status=500)
